...
void draw_clock(struct Size current) {
    ...
    size_t len = ((8 * 6) + 8 + (7 * 2));

    int x = current.w * 0.45, y = current.h * 0.15;
    
    for (int i = 0; i < 6; i++) {
        draw_text_checkpoint(x, y, format(len, "%s  %s  %s  %s  %s  %s  %s  %s", digits[times[0]][i], digits[times[1]][i], digits[10][i], digits[times[2]][i], digits[times[3]][i], digits[10][i], digits[times[4]][i], digits[times[5]][i]), FONT_COLOR, current);
        y += FONT_H;
    }
}
...
void update_logs(struct Size _current) {
    ...
    if (now > next_time) {
        add_log_line();
        next_time = now + 0.05 + ((rand() % 400) / 1000.0);
    }
}
...
static void get_daily_quote(char *out) {
    ...
    snprintf(out, QUOTE_WIDTH + 1, "%-53s", q);
}
...
void draw_black_hole(struct Size current) {
    char quote[QUOTE_WIDTH];
    get_daily_quote(quote);
    draw_text_checkpoint(current.w * 0.35, current.h * 0.35, format(sizeof(black_hole) + QUOTE_WIDTH, black_hole, quote), FONT_COLOR, current);
}
